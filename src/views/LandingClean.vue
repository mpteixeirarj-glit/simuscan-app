  ...
  <div class="landing-clean">
    ...
    <section class="hero">
      <div class="hero-inner">
        <p class="hero-eyebrow">Simulador Educacional de TC</p>
        <h1 class="hero-h1">
        ...
        <div class="hero-ctas">
          <RouterLink to="/login" class="cta-pill-primary">Começar agora</RouterLink>
          <button class="cta-pill-ghost" @click="scrollTo('funcionalidades')">Ver funcionalidades</button>
        </div>
      </div>
    ...
    <section class="section" id="sistemas">
      ...
      <div class="sistemas-grid">
        <div class="sistema-card gama-card">
          <div class="sys-top-bar gama-bar"></div>
          ...
          <RouterLink to="/login" class="sys-btn gama-btn">Acessar</RouterLink>
        </div>
        <div class="sistema-card sigma-on-card">
          <div class="sys-top-bar sigma-on-bar"></div>
          <div class="sys-icon-wrap">
        ...
        <div class="sistema-card sigma-old-card">
          <div class="sys-top-bar sigma-old-bar"></div>
          <div class="sys-icon-wrap">
        ...
        <div class="sistema-card kappa-card">
          <div class="sys-top-bar kappa-bar"></div>
          <div class="sys-icon-wrap">
    ...
    <footer class="footer">
      <div class="footer-inner">
        <div class="footer-brand">
          ...
          <p class="footer-desc">Simulador educacional de Tomografia Computadorizada.</p>
...
.landing-clean { min-height: 100vh; background: #fafafa; color: #1c1c2e; font-family: 'Inter', system-ui, sans-serif; }

/* DEVICE NOTICE */
.device-notice { background: #f0f4ff; border-bottom: 1px solid #dde4f5; padding: 10px 24px; text-align: center; font-size: 13px; color: #666; }
.device-notice strong { color: #3498db; font-weight: 700; }
.device-notice kbd { display: inline-block; padding: 1px 6px; border: 1px solid #ccc; border-radius: 3px; font-size: 12px; background: #f5f5f5; }

/* NAV */
.nav { position: sticky; top: 0; z-index: 50; background: rgba(250,250,250,0.96); backdrop-filter: blur(10px); border-bottom: 1px solid #ebebeb; }
...
.logo-text { font-family: 'Rajdhani', sans-serif; font-weight: 700; font-size: 1.4rem; }
.logo-text .p { color: #e91e8c; }
.logo-text .b { color: #3498db; }
.nav-links { display: flex; gap: 2rem; }
.nav-links a { font-size: .85rem; color: #888; text-decoration: none; font-weight: 500; transition: color .2s; }
.nav-links a:hover { color: #1c1c2e; }
.btn-nav { padding: .4rem 1.1rem; border: 1.5px solid #e8e8e8; border-radius: 100px; color: #555; font-size: .88rem; font-weight: 600; text-decoration: none; transition: border-color .2s, color .2s; }
.btn-nav:hover { border-color: #3498db; color: #3498db; }

/* HERO */
.hero { max-width: 1100px; margin: 0 auto; padding: 5rem 2rem; display: grid; grid-template-columns: 1fr 1fr; gap: 4rem; align-items: center; }
.hero-eyebrow { font-size: .68rem; font-weight: 700; letter-spacing: .14em; text-transform: uppercase; color: #3498db; margin-bottom: .8rem; }
.hero-h1 { font-family: 'Rajdhani', sans-serif; font-weight: 700; font-size: clamp(2rem, 4vw, 3rem); line-height: 1.1; color: #1c1c2e; margin-bottom: 1.2rem; }
.hero-h1 .pink { color: #e91e8c; }
.hero-h1 .blue { color: #3498db; }
.hero-sub { font-size: .92rem; color: #888; line-height: 1.7; margin-bottom: 2rem; }
.hero-ctas { display: flex; gap: .8rem; flex-wrap: wrap; }
.cta-pill-primary { padding: .75rem 1.8rem; background: #e91e8c; border: none; border-radius: 100px; color: #fff; font-family: 'Rajdhani', sans-serif; font-weight: 700; font-size: .95rem; letter-spacing: .04em; text-decoration: none; transition: opacity .2s; }
.cta-pill-primary:hover { opacity: .85; }
.cta-pill-ghost { padding: .75rem 1.8rem; background: none; border: 1.5px solid #ddd; border-radius: 100px; color: #888; font-size: .95rem; cursor: pointer; transition: border-color .2s, color .2s; }
.cta-pill-ghost:hover { border-color: #3498db; color: #3498db; }
.hero-media { border-radius: 16px; overflow: hidden; border: 1px solid #ebebeb; }
.hero-video, .hero-img { width: 100%; height: auto; display: block; }

/* DIVIDER */
.divider { max-width: 1100px; margin: 0 auto; height: 1px; background: #ebebeb; }

/* SECTIONS */
.section { max-width: 1100px; margin: 0 auto; padding: 5rem 2rem; }
.eyebrow { font-size: .68rem; font-weight: 700; letter-spacing: .14em; text-transform: uppercase; color: #3498db; margin-bottom: .8rem; }
.section-title { font-family: 'Rajdhani', sans-serif; font-weight: 700; font-size: clamp(1.6rem, 3vw, 2.2rem); color: #1c1c2e; margin-bottom: 2.5rem; }

/* FEATURE CARDS */
.cards-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(260px, 1fr)); gap: 1.5rem; }
.card { background: #fff; border: 1px solid #ebebeb; border-radius: 12px; overflow: hidden; transition: border-color .2s, box-shadow .2s; }
.card:hover { border-color: #e91e8c; box-shadow: 0 8px 32px rgba(233,30,140,0.08); }
.card-img-wrap { width: 100%; height: 170px; overflow: hidden; }
.card-img-wrap img { width: 100%; height: 100%; object-fit: cover; }
.card-body { padding: 1.5rem; }
.card-icon { width: 48px; height: 48px; margin-bottom: .8rem; }
.icon-img { width: 48px; height: 48px; object-fit: contain; }
.card-body h3 { font-size: .95rem; font-weight: 700; color: #1c1c2e; margin-bottom: .4rem; }
.card-body p { font-size: .85rem; color: #888; line-height: 1.6; }

/* SISTEMAS — colored top border */
.sistemas-grid { display: grid; grid-template-columns: repeat(4, 1fr); gap: 1.5rem; }
.sistema-card { background: #fff; border: 1px solid #ebebeb; border-radius: 12px; overflow: hidden; display: flex; flex-direction: column; align-items: center; padding: 2rem 1.5rem; text-align: center; transition: box-shadow .2s; }
.sigma-on-card, .sigma-old-card, .kappa-card { opacity: .8; }
.sistema-card:hover { box-shadow: 0 8px 32px rgba(0,0,0,0.08); opacity: 1; }
.sys-top-bar { width: 100%; height: 4px; margin: -2rem -1.5rem 1.5rem; width: calc(100% + 3rem); }
.gama-bar { background: #3498db; }
.sigma-on-bar { background: #2ecc71; }
.sigma-old-bar { background: #7f8fc9; }
.kappa-bar { background: #9e3d52; }
.sys-icon-wrap { width: 72px; height: 72px; display: flex; align-items: center; justify-content: center; margin-bottom: 1rem; }
.sys-icon { width: 64px; height: 64px; object-fit: contain; border-radius: 10px; }
.sys-badge { display: inline-block; font-size: 11px; font-weight: 700; letter-spacing: 1px; text-transform: uppercase; padding: 2px 10px; border-radius: 100px; margin-bottom: .8rem; }
.sys-badge.available { background: rgba(46,204,113,0.1); color: #27ae60; border: 1px solid rgba(46,204,113,0.25); }
.sys-badge.soon { background: #f5f5f5; color: #aaa; border: 1px solid #e8e8e8; }
.sys-name { font-size: .95rem; font-weight: 700; color: #1c1c2e; margin-bottom: .3rem; }
.sys-subtitle { font-size: 11px; font-style: italic; color: #bbb; margin-bottom: .5rem; }
.sys-desc { font-size: .82rem; color: #888; line-height: 1.6; margin-bottom: 1rem; }
.sys-btn { display: inline-block; padding: .55rem 1.4rem; background: #3498db; color: #fff; border-radius: 100px; font-weight: 700; font-size: .88rem; text-decoration: none; transition: background .2s; margin-top: auto; }
.sys-btn:hover { background: #2980b9; }
.sys-btn-disabled { padding: .55rem 1.4rem; background: #f5f5f5; color: #ccc; border: 1px solid #e8e8e8; border-radius: 100px; font-size: .88rem; cursor: not-allowed; width: 100%; margin-top: auto; }

/* PLANOS */
.planos-grid { display: grid; grid-template-columns: repeat(2, 1fr); gap: 2rem; max-width: 720px; margin: 0 auto; }
.plano-card { background: #fff; border: 1px solid #ebebeb; border-radius: 12px; padding: 2rem; display: flex; flex-direction: column; gap: 1rem; position: relative; transition: box-shadow .2s; }
.plano-card.destaque { border-color: #3498db; box-shadow: 0 4px 24px rgba(52,152,219,0.1); }
.plano-card:hover { box-shadow: 0 8px 32px rgba(0,0,0,0.08); }
.plano-badge { position: absolute; top: -12px; left: 50%; transform: translateX(-50%); background: #3498db; color: #fff; font-size: .65rem; font-weight: 800; letter-spacing: .1em; text-transform: uppercase; padding: 3px 12px; border-radius: 100px; white-space: nowrap; }
.plano-card h3 { font-size: 1.1rem; font-weight: 700; color: #1c1c2e; }
.preco { font-family: 'Rajdhani', sans-serif; font-size: 2.4rem; font-weight: 700; color: #3498db; }
.preco span { font-size: 1rem; color: #aaa; }
.economy-badge { display: inline-block; font-size: 11px; font-weight: 700; padding: 2px 10px; border-radius: 100px; background: rgba(46,204,113,0.08); color: #27ae60; border: 1px solid rgba(46,204,113,0.2); }
.annual-note { font-size: 12px; color: #aaa; }
.plano-card ul { list-style: none; display: flex; flex-direction: column; gap: .5rem; flex: 1; }
.plano-card ul li { font-size: .88rem; color: #666; }
.plano-card ul li::before { content: '✓ '; color: #27ae60; font-weight: 700; }
.plano-btn { padding: .75rem; background: #f5f5f5; border: none; border-radius: 100px; color: #555; font-family: 'Rajdhani', sans-serif; font-weight: 700; font-size: .95rem; cursor: pointer; transition: background .2s; }
.plano-btn:hover { background: #ebebeb; }
.destaque-btn { background: #3498db; color: #fff; }
.destaque-btn:hover { background: #2980b9; }

/* AI DISCLOSURE */
.ai-disclosure { background: #f5f6fa; border-top: 1px solid #ebebeb; border-bottom: 1px solid #ebebeb; padding: 2rem; }
...
.ai-title { font-size: 14px; font-weight: 700; color: #1c1c2e; margin-bottom: 8px; }
.ai-text { font-size: 13px; color: #888; line-height: 1.7; margin-bottom: 6px; }

/* FOOTER */
.footer { background: #1c1c2e; color: #e8e8ec; padding: 4rem 2rem 0; }
...
.footer-desc { font-size: .82rem; color: rgba(255,255,255,0.45); line-height: 1.6; margin-bottom: 1rem; }
...
.footer-col h4 { font-size: .72rem; font-weight: 700; letter-spacing: .1em; text-transform: uppercase; color: rgba(255,255,255,0.35); margin-bottom: .8rem; }
.footer-col ul { list-style: none; display: flex; flex-direction: column; gap: .5rem; }
.footer-col ul li a, .footer-col ul a { font-size: .85rem; color: rgba(255,255,255,0.45); text-decoration: none; transition: color .2s; }
.footer-col ul li a:hover, .footer-col ul a:hover { color: #e8e8ec; }
.footer-bottom { border-top: 1px solid rgba(255,255,255,0.07); max-width: 1100px; margin: 0 auto; padding: 1.2rem 0; display: flex; justify-content: space-between; font-size: .78rem; color: rgba(255,255,255,0.35); flex-wrap: wrap; gap: .5rem; }
.footer-version { color: #3498db; font-size: 11px; opacity: .7; }
.footer-legal-bottom { padding: 10px 0; text-align: center; border-top: 1px solid rgba(255,255,255,0.04); }
.footer-legal-bottom p { font-size: 12px; color: rgba(255,255,255,0.18); margin: 0; }
.footer-legal-final { padding: 8px 0 16px; text-align: center; }
.footer-legal-final p { font-size: 11px; color: rgba(255,255,255,0.1); margin: 0; }
